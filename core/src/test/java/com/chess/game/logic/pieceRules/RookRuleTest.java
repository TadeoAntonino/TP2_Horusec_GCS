...
import com.chess.game.logic.pieceRules.RookRule;
...
class RookRuleTest {
    ...
    @ParameterizedTest(name = "Validate isMoved false for new {0} Rook")
    @MethodSource("provideArgumentsOf_initialIsMovedTest")
    void initialIsMovedTest(TeamEnum team) {
        //Arrange
        RookRule pieceRuleUnderTest = new RookRule(team);
        //Act
    ...
    @ParameterizedTest(name = "Validate isMoved true for moved {0} Rook")
    ...
    @ParameterizedTest(name = "Validate movement paths for {0} Rook in position {1}")
